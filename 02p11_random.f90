...
REAL(dl), PRIVATE             :: zero = 0.0, half = 0.5, one = 1.0, two = 2.0
PRIVATE                   :: integral
...
!     Local variables
REAL(dl)     :: s = 0.449871, t = -0.386595, a = 0.19600, b = 0.25472,           &
...
DOUBLE PRECISION FUNCTION random_gamma(s, first)
...
DOUBLE PRECISION FUNCTION random_gamma1(s, first)
...
!     Local variables
REAL(dl)    :: b, h, d, r, g, f, x, sixty4 = 64.0, three = 3.0, pt75 = 0.75
SAVE b, h
...
DOUBLE PRECISION FUNCTION random_gamma2(s, first)
...
!     Local variables
REAL(dl)                :: a, p, c, uf, vr, d, r, x, w
REAL(dl) ::         vsmall, vlarge
...
DOUBLE PRECISION FUNCTION random_chisq(ndf, first)
...
DOUBLE PRECISION FUNCTION random_exponential( )
...
DOUBLE PRECISION FUNCTION random_Weibull(a)
...
DOUBLE PRECISION FUNCTION random_beta(aa, bb, first)
...
IMPLICIT NONE
REAL(dl), INTENT(IN)    :: aa, bb
LOGICAL, INTENT(IN) :: first

!     Local variables
REAL(dl), PARAMETER     :: aln4 = 1.3862944
REAL(dl)                :: a, b, d, f, h, t, c, g, r, s, x, y, z
LOGICAL             :: swap
REAL(dl) ::         vsmall, vlarge
...
DOUBLE PRECISION FUNCTION random_t(m)
...
!     Local variables
REAL(dl)      :: s, c, a, f, g, r, x, v, three = 3.0, four = 4.0, quart = 0.25,   &
...
INTEGER, INTENT(IN)  :: n
REAL(dl), INTENT(IN)     :: h(n), d(n*(n+1)/2)
REAL(dl), INTENT(OUT)    :: f(n*(n+1)/2), x(n)
LOGICAL, INTENT(IN)  :: first
...
DOUBLE PRECISION FUNCTION random_inv_gauss(h, b, first)
...
IMPLICIT NONE
REAL(dl), INTENT(IN)    :: h, b
LOGICAL, INTENT(IN) :: first

REAL(dl) ::         vsmall, vlarge

!     Local variables
REAL(dl)            :: a, c, d, e, ym, xm, r, w, r1, r2, x, quart = 0.25
SAVE a, c, d, e
vsmall = TINY(1.0); vlarge = HUGE(1.0)
...
!     ..
!     .. Local Scalars ..
REAL(dl) a0, a1, a2, a3, a4, a5, a6, a7, b1, b2, c, c0, c1, c2, c3, d, del,      &
     difmuk, e, fk, fx, fy, g, omega, p, p0, px, py, q, s, t, u, v, x, xx
INTEGER j, k, kflag, l, m
LOGICAL :: full_init
!     ..
!     .. Local Arrays ..
REAL(dl) fact(10), pp(35)
!     ..
!     .. Intrinsic Functions ..
INTRINSIC ABS, LOG, EXP, FLOAT, IDINT, MAX, MIN, SIGN, SQRT
!     ..
!     .. Data statements ..
...
!             PROBABILITIES FK WHENEVER K >= M(MU). L=IDINT(MU-1.1484)
...
INTEGER             :: r0, ru, rd
REAL(dl)                :: odds_ratio, u, p_r, pd, pu, zero = 0.0, one = 1.0
...
!     ..
!     .. Scalar Arguments ..
REAL(dl), INTENT(IN)    :: pp
INTEGER, INTENT(IN) :: n
LOGICAL, INTENT(IN) :: first
!     ..
!     .. Local Scalars ..
REAL(dl)    :: al, alv, amaxp, c, f, f1, f2, ffm, fm, g, p, p1, p2, p3, p4, q,  &
            qn, r, u, v, w, w2, x, x1, x2, xl, xll, xlr, xm, xnp, xnpq, xr, &
            ynorm, z, z2
REAL(dl)    :: zero = 0.0, half = 0.5, one = 1.0
...
IMPLICIT NONE
REAL(dl), INTENT(IN)   :: sk, p

!     Local variables
! THE PARAMETER ULN = -LOG(MACHINE'S SMALLEST REAL NUMBER).

REAL(dl), PARAMETER    :: h = 0.7
REAL(dl)               :: q, x, st, uln, v, r, s, y, g
INTEGER            :: k, i, n
REAL(dl) ::         vsmall, vlarge
...
DOUBLE PRECISION FUNCTION random_von_Mises(k, first, ier)
...
INTEGER              :: nk, j, n
REAL(dl), PARAMETER      :: pi = 3.14159265
REAL(dl)                 :: p(20), theta(0:20), sump, r, th, lambda, rlast
DOUBLE PRECISION     :: dk
...
DOUBLE PRECISION FUNCTION random_Cauchy()
IMPLICIT NONE
REAL(dl) ::         vsmall, vlarge
