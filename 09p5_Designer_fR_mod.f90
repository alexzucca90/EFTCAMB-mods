...
module EFTCAMB_designer_fR_mod
    ...
    public EFTCAMB_fR_designer_mod
    ...
    type, extends ( EFTCAMB_designer_model ) :: EFTCAMB_fR_designer_mod
        ...
        integer  :: EFTxDE                                                !< Model selection flag for designer f(R) X DE.

        ! the pure EFT functions:
        class( parametrized_function_1D ), allocatable :: DesfRxDE      !< The pure EFT function w_DE.
...
!real(dl) ::
    ...
    end type EFTCAMB_fR_designer_mod
...
contains
    ...
    subroutine EFTCAMBDesignerFRReadModelSelectionFromFile( self, Ini )
        ...
        class(EFTCAMB_fR_designer_mod)  :: self   !< the base class
    ...
    subroutine EFTCAMBDesignerFRInitModelParameters( self, array )
        ...
        call self%DesfRxDE%init_parameters(temp)
    ...
    subroutine EFTCAMBDesignerFRInitModelParametersFromFile( self, Ini )
        ...
        call self%DesfRxDE%init_from_file( Ini )
    ...
    contains
        ...
        subroutine derivs( num_eq, x, y, ydot )
            ...
            !> AZ MOD START: adding some new parameters for X_DE (X and its derivatives w.r.t. ln a)
            real(dl) :: EFT_X, EFT_Xp, EFT_Xpp, EFT_Xppp
            !> AZ MOD END
            ...
            !>  fractional dark energy density and its derivatives w.r.t. x = ln a
            EFT_X       = self%DesfRxDE%value(a)
            EFT_Xp      = self%DesfRxDE%first_derivative(a)*a
            EFT_Xpp     = self%DesfRxDE%second_derivative(a)*a**2 + self%DesfRxDE%first_derivative(a)*a
            EFT_Xppp    = self%DesfRxDE%third_derivative(a)*a**3 + 3._dl*self%DesfRxDE%second_derivative(a)*a**2 + self%DesfRxDE%first_derivative(a)*a
            !> AZ MOD END
            ...
            !> AZ MOD START: modufying the contributions of X, Xp in E
            ...
            !> AZ MOD START: X contribution in E2prime and E3prime
    ...
    subroutine EFTCAMBDesignerFRComputeParametersNumber( self )
        ...
        !> AZ MOD START: replacing wDE with xDE
        self%parameter_number = self%parameter_number +self%DesfRxDE%parameter_number
        !> AZ MOD END
    ...
    subroutine EFTCAMBDesignerFRFeedback( self, print_params )
        ...
        call self%DesfRxDE%feedback( print_params )
        !> AZ MOD END
    ...
    subroutine EFTCAMBDesignerFRParameterNames( self, i, name )
        ...
        ! the other parameters are the w_DE parameters:
            ...
            call self%DesfRxDE%parameter_names( i-1, name )
            return
        end if
        !> AZ MOD END
    ...
    subroutine EFTCAMBDesignerFRParameterValues( self, i, value )
        ...
        ! the other parameters are the x_DE parameters:
            ...
            !> AZ MOD START: replacing wDE with xDE
            call self%DesfRxDE%parameter_value( i-1, value )
            !> AZ MOD END
            return
        end if
